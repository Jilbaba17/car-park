...
class RecoveryController extends Controller {
	...
	public function actionRequest() {
		...
		$model = \Yii::createObject([
			'class' => RecoveryForm::className(),'scenario' => RecoveryForm::SCENARIO_REQUEST
		]);
	...
	public function actionReset($id, $code) {
		...
		$token = $this->finder->findToken([
			'user_id' => $id,'code' => $code,'type' => Token::TYPE_RECOVERY
		])->one();
		...
		if ($token === null || $token->isExpired || $token->user === null) {
			...
			return $this->render('/message', [
				'title' => \Yii::t('user', 'Invalid or expired link'),'module' => $this->module
			]);
		}
		
		/** @var RecoveryForm $model */
		$model = \Yii::createObject([
			'class' => RecoveryForm::className(),'scenario' => RecoveryForm::SCENARIO_RESET
		]);
		$event->setForm($model);
...
// 			return $this->render('/message', [
// 				'title' => \Yii::t('user', 'Password has been changed'),'module' => $this->module
// 			]);
			return $this->redirect([
				'/user/login'
			]);
